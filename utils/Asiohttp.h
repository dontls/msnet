...
class Asiohttp {
 ...
 public:
  ...
  int post(const char* page, const std::string& data,
 ...
 private:
  ...
  // POST GET
  int doNetAccess(const char* method, const char* page,
                  asio::ip::tcp::socket& socket, std::string data) {
    asio::ip::tcp::resolver resolver(_ioService);
  ...
  int doNetReponse(asio::ip::tcp::socket& socket, std::string& respData) {
    ...
    if (response.size()) {
      ...
      respData =
          std::string(std::istreambuf_iterator<char>(response_stream), eos);
    }
