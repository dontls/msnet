...
public:
    ...
    bool init(int nSamplePerSec, int nChn, int nBitsPerSample, int nOfmt)
    {
        ...
        //一帧 1024个 sample。采样率 Samplerate 44100KHz，每秒44100个sample, 所以 根据公式   音频帧的播放时间 =
        //一个AAC帧对应的采样样本的个数 / 采样频率 当前AAC一帧的播放时间是 = 1024 * 1000000 / 44100 = 22320ns(单位为ns)
    ...
    unsigned char* encode(unsigned char* pcmData, long dataLen, int& accLen)
    {
        ...
        int nEncDataLen = 0;
        for (;;) {
            nEncDataLen = faacEncEncode(_faacEnc, ( int* )pcmData, nInputSample, _pOutputBytes, _maxOutputBytes);
            if (nEncDataLen < 1) {
                continue;
            }
            break;
