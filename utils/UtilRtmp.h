...
// 发送视频的sps和pps信息
int SendVideoSpsPps(RTMP* r, unsigned char* pps, int pps_len,
                    unsigned char* sps, int sps_len, uint32_t dts) {
...
// 发送视频的sps和pps、vps信息
int SendVideoSpsPpsVps(RTMP* r, unsigned char* pps, int pps_len,
                       unsigned char* sps, int sps_len, unsigned char* vps,
  ...
  if (1 == RTMP_IsConnected(r))
    nRet = RTMP_SendPacket(r, packet,
                           TRUE);  // 1为放进发送队列,0是不放进发送队列,直接发送
  return nRet;
}

// 发送RTMP包
int SendPacket(RTMP* r, unsigned int nPacketType, unsigned char* data,
               unsigned int size, uint32_t nTimestamp) {
  /*分配包内存和初始化,len为包体长度*/
  ...
  int nRet = 0;
  if (1 == RTMP_IsConnected(r)) {
    nRet = RTMP_SendPacket(
        r, packet, FALSE); /*TRUE为放进发送队列,FALSE是不放进发送队列,直接发送*/
  }
  return nRet;
}
  ...
  switch (nut) {
    ...
    case avc::NALU_TYPE_IDR: {
      ...
      if (nut == avc::NALU_TYPE_IDR) {
        ...
        if (SendVideoSpsPps(r, (unsigned char*)avcExt.pps.c_str(),
                            avcExt.pps.length(),
                            (unsigned char*)avcExt.sps.c_str(),
                            avcExt.sps.length(), dts) == 0) {
          break;
        }
      } else {
  ...
  switch (nut) {
    ...
    case hevc::NALU_TYPE_CODED_SLICE_IDR: {
      ...
      if (nut == hevc::NALU_TYPE_CODED_SLICE_IDR) {
        ...
        if (SendVideoSpsPpsVps(
                r, (unsigned char*)avcExt.pps.c_str(), avcExt.pps.length(),
                (unsigned char*)avcExt.sps.c_str(), avcExt.sps.length(),
                (unsigned char*)avcExt.vps.c_str(), avcExt.vps.length(),
                dts) == 0) {
          break;
        }
      } else {
...
// 发送AAC状态
int SendAccAudioframe(RTMP* r, unsigned char* data, int len, uint32_t dts,
                      bool isAudio = true) {
