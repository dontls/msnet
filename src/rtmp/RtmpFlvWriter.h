...
class RtmpFlvWriter {
 private:
  ...
  int sendPacket(unsigned int nPacketType, const char* body, int bodyLen,
                 uint32_t abstamp) {
    if (!_rtmp) {
      return -1;
    }
    _packet.Realloc(RTMP_HEAD_SIZE + bodyLen);
    char* tBuffer = _packet.Bytes();
    ...
    packet->m_packetType =
        nPacketType; /*此处为类型有两种一种是音频,一种是视频*/
 ...
 public:
  ...
  void setAvcRtmpPacket(std::string flvTag, uint32_t abstamp) {
    sendPacket(RTMP_PACKET_TYPE_VIDEO, flvTag.c_str(), flvTag.length(),
               abstamp);
  }

  void setAccRtmpPacket(std::string flvTag, uint32_t abstamp) {
    sendPacket(RTMP_PACKET_TYPE_AUDIO, flvTag.c_str(), flvTag.length(),
               abstamp);
  }
};

using RtmpFlvWriter_Ptr = std::shared_ptr<RtmpFlvWriter>;
