...
inline void doNginxStatAccess(XmlConfig* conf, const char* method,
                              const char* page, const char* param) {
  Asiohttp http;
  ...
  rapidxml::xml_node<>* streamNode = root->first_node("server")
                                         ->first_node("application")
                                         ->first_node("live")
                                         ->first_node("stream");
  if (streamNode == NULL) {
    return;
  ...
  while (streamNode) {
    ...
    std::cout << "name " << st.name << " bytes_out " << st.bytesOut
              << " nclients " << st.clis << "\n";
